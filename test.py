...
from env import DBNAME, DBHOST, DBPASS, DBUSER, API_KEY
...
class NimbusTests(unittest.TestCase):
    ...
    def test_db_connection(self):
        db = Database(host=DBHOST, database=DBNAME, user=DBUSER, password=DBPASS)
        try:
            db.connect()
            db.disconnect()
        except MySQLError:
            self.fail(f"Unable to connect to database")

    def test_distributed_ai_service_connection(self):
        genai.configure(api_key=API_KEY)
        self.assertNotEqual(
            len(list(genai.list_models())),
            0,
            "Unable to connect to distributed AI service",
        )
    ...
    def test_register_user_exception(self):
        ...
        with unittest.mock.patch(
            "entry.user_manager.register_user", side_effect=Exception("Test Exception")
        ):
            data = {"username": "testuser"}
            response = self.client.post("/register_user", data=data)
            self.assertEqual(response.status_code, 200)
    ...
    def test_get_user_exception(self):
        ...
        with patch(
            "entry.user_manager.get_user_id_by_username",
    ...
    def test_get_definition_success_with_context(self):
        ...
        with patch("entry.faq_manager.add_faq") as mock_add_faq, patch(
            "entry.context_window.get_last_response"
        ) as mock_get_last_response, patch(
            "entry.get_term_meaning"
        ) as mock_get_term_meaning, patch(
            "entry.context_window.add_context_window"
        ) as mock_add_context_window:
            ...
            response = self.client.get(
                "/definition", data={"term": "testterm"}, query_string={"user_id": "1"}
            )
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.json["status"], "success")
    ...
    def test_get_definition_missing_term(self):
        ...
        response = self.client.get(
            "/definition", data={}, query_string={"user_id": "1"}
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json["status"], "error")
        self.assertIn("message", response.json)
    ...
    def test_get_definition_exception(self):
        ...
        with patch(
            "entry.faq_manager.add_faq", side_effect=Exception("Test Exception")
        ):
            response = self.client.get(
                "/definition", data={"term": "testterm"}, query_string={"user_id": "1"}
            )
    ...
    def test_get_risk_assessment_exception(self):
        ...
        with patch(
            "entry.risk_assessment_questions", side_effect=Exception("Test Exception")
        ):
            # Test the GET request
            response = self.client.get("/get_risk_assessment")
            self.assertEqual(response.status_code, 200)
    ...
    def test_assign_risk_level_success(self):
        ...
        with patch("entry.risk_level_assignment") as mock_risk_level_assignment, patch(
            "entry.user_manager.update_user"
        ) as mock_update_user:
    ...
    def test_assign_risk_level_exception(self):
        ...
        with patch(
            "entry.risk_level_assignment", side_effect=Exception("Test Exception")
        ):
            data = {
                "user_id": "1",
                "quiz": ["Question 1", "Question 2"],
    ...
    def test_get_investment_options_success(self):
        ...
        with patch("entry.user_manager.get_risk_level") as mock_get_risk_level, patch(
            "entry.investment_option_generation"
        ) as mock_investment_option_generation:
    ...
    def test_get_investment_options_exception(self):
        ...
        with patch(
            "entry.user_manager.get_risk_level", side_effect=Exception("Test Exception")
        ):
            data = {"user_id": "1"}
            response = self.client.post("/investment_options", data=data)
            self.assertEqual(response.status_code, 200)
    ...
    def test_generate_portfolio_success(self):
        ...
        with patch("entry.user_manager.get_risk_level") as mock_get_risk_level, patch(
            "entry.generate_diversified_portfolio"
        ) as mock_generate_portfolio:
            ...
            response = self.client.get(
                "/diverse_portfolio", query_string={"user_id": "1"}
            )
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.json["status"], "success")
    ...
    def test_generate_portfolio_missing_user_id(self):
        ...
        )  # Automatically assumes a risk level of medium
        self.assertIn("portfolio", response.json)
    ...
    def test_get_finance_quiz_success(self):
        ...
        with patch("entry.user_manager.get_risk_level") as mock_get_risk_level, patch(
            "entry.generate_quizzes"
        ) as mock_generate_quizzes:
    ...
    def test_get_faqs_success(self):
        ...
        with patch("entry.faq_manager.get_faq") as mock_get_faq, patch(
            "entry.return_faqs"
        ) as mock_return_faqs:
    ...
    def test_get_faqs_exception(self):
        ...
        with patch(
            "entry.faq_manager.get_faq", side_effect=Exception("Test Exception")
        ):
            response = self.client.get("/get_faqs")
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.json["status"], "error")
    ...
    def test_allocate_income_success(self):
        ...
        with patch("entry.ai_recommendations") as mock_ai_recommendations:
            mock_ai_recommendations.return_value = {
                "investment": 1000,
                "essentials": 2000,
                "discretionary": 500,
            }
            ...
            self.assertEqual(
                response.json["updated_data"],
                {"investment": 1000, "essentials": 2000, "discretionary": 500},
            )
    ...
    def test_allocate_income_missing_data(self):
        ...
        )  # Automatically assumes a risk level of medium, and sets values to 0
